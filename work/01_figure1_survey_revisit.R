...
# Last edited by: Anne, Feb 2021 for figure proofs
...
g = ggplot(map, aes(long, lat, group=group)) + # the data
    geom_polygon(color="white", lwd=.04, aes(fill=cut_diff)) + #polygons w fill + outline
...
ggplot2::ggsave(paste0(git_path, "/figures/raw/Figure_1a.pdf"), plot=g, 
...
g = ggplot(surveys, aes(gdppc, tot)) + geom_point(alpha=0.2) + 
    ...
    annotation_logticks(short = unit(-0.5,"mm"), mid = unit(-1.2,"mm"), 
                        long = unit(-2,"mm"), sides = "b", color = "#333333") + 
...
ggplot2::ggsave(paste0(git_path, "/figures/raw/Figure_1b.pdf"), plot=g, 
...
ggplot2::ggsave(paste0(git_path, "/figures/raw/Figure_1c.pdf"), plot=g, 
                width=2.25, height=2.25)
...
ag$years = apply(ag[, 2:6], 1, max_na)
...
ag$years_since = 2018-ag$years
ag = merge(ag[, c("country", "iso3", "years", "years_since")], 
...
map[map$id=="SJM", "id"] = "NOR"
map[map$id=="GRL", "id"] = "DNK"
map = merge(map, ag[, c("iso3", "years", "years_since")], by.x="id", by.y="iso3", all=T)
map = map[order(map$group, map$piece, map$order),]
...
g = ggplot(map, aes(long, lat, group=group)) + # the data
    geom_polygon(color="white", lwd=.04, aes(fill=cut_year)) + #polygons w fill + outline
    scale_fill_discrete(palette=cols, breaks=diff, limits=lim) + 
    geom_polygon(aes(long, lat, group=group), fill="black", color="white", lwd=.04, 
...
r2 = format(round(summary(model)$r.squared, 2), nsmall = 2)
lab = paste("r^2 =='", r2, "'")
g = ggplot(ag, aes(gdppc, years_since)) + geom_point(alpha=0.2, shape=16) + 
    geom_smooth(method = "lm", se = F, alpha = 0.4, size = 0.5) + 
    annotate("text", x = 100, y = 41, label = lab, color = "grey46", hjust = 0, 
             vjust = 1, size = 12/2.5, family = "sans", parse = T) + ylim(0, 25) +
    scale_x_log10(breaks = c(100, 1000, 10000, 50000), limits=c(100, 50000)) +
    annotation_logticks(short = unit(-0.5,"mm"), mid = unit(-1.2,"mm"), 
                        long = unit(-2,"mm"), sides = "b", color = "#333333") + 
    coord_cartesian(clip="off") +
...
ggplot2::ggsave(paste0(git_path, "/figures/raw/Figure_1e.pdf"), plot=g, 
                width=2.25, height=2.25)

g = panel(ag[!is.infinite(ag$years_since), 
              c("polity2", "years_since")], betal=T, font="Times New Roman", size=15, ybounds=c(0, 25)) + 
    xlab("Polity") + ylab("Years Since") 
ggplot2::ggsave(paste0(git_path, "/figures/raw/Figure_1f.pdf"), plot=g, 
                width=2.25, height=2.25)
...
r2 = format(round(summary(model)$r.squared, 2), nsmall = 2)
lab = paste("r^2 =='", r2, "'")
g = ggplot(pop, aes(gdppc, years_since)) + geom_point(alpha=0.2, shape=16) + 
    geom_smooth(method = "lm", se = F, alpha = 0.4, size = 0.5) + 
    annotate("text", x = 100, y = 41, label = lab, color = "grey46", hjust = 0, 
             vjust = 1, size = 12/2.5, family = "sans", parse = T) + ylim(0, 25) +
    scale_x_log10(breaks = c(100, 1000, 10000, 50000), limits=c(100, 50000)) +
    ...
    theme(panel.background = element_blank(),
          ...
          text=element_text(size=15,  family="Times New Roman")) +
    xlab("Log GDP per capita") + ylab("Years Since")
ggplot2::ggsave(paste0(git_path, "/figures/raw/Figure_1h.pdf"), plot=g, 
                width=2.25, height=2.25)
...
g = panel(pop[!is.infinite(pop$years_since), 
          ...
          ybounds=c(0, 25)) + 
    xlab("Polity") + ylab("Years Since")
ggplot2::ggsave(paste0(git_path, "/figures/raw/Figure_1i.pdf"), plot=g, 
                width=2.25, height=2.25)
