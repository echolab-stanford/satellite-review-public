...
########################################################
# Global pixel correlation 2dbins
########################################################
...
#global_cor1 <- cor(df$worldpop, df$landscan, method="pearson", use="complete.obs")
#global_cor2 <- cor(df$ghsl, df$landscan, method="pearson", use="complete.obs")
#global_cor3 <- cor(df$ghsl, df$worldpop, method="pearson", use="complete.obs")

#set.seed(42)
#subset_df <- df.rmna %>% sample_frac(0.01)
...
g = ggplot(merged, aes(long, lat, group=group)) + # the data
    ...
    #geom_polygon(aes(long, lat, group=group), fill="black", color="white", lwd=.025, 
    #             data=map[map$years < 1976 | (map$id=="ESH" & !is.na(map$id)), ]) +
    #geom_polygon(color="darkgrey", fill=NA, lwd=0.07, data=world) +
